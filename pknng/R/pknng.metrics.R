...
correl<-.Call("correlation_metric",data, c(dim(data)[1],dim(data)[2]),DUP=F)
...
euc<-.Call("euc_metric",data, c(dim(data)[1],dim(data)[2]),DUP=F)
#par[0] rows (dimension of Data), par[1] cols (amount of Data)
return(euc)
}
...
euc<-.Call("histo_fun",data,length(data),DUP=F)
#par[0] rows (dimension of Data), par[1] cols (amount of Data)
return(euc)
}

mi_metric<-function(data){
euc<-.Call("mi_metric",data,c(dim(data)[2],dim(data)[1],0,1),DUP=F)
return(euc)
}

mutualInf<-function(data){
euc<-.Call("mutualInfo",data,c(dim(data)[2],dim(data)[1],0),DUP=F)
return(euc)
}

mi_estimator<-function(data){
## data rows dimension, col amount of data
euc<-.Call("mi_estimator",data,c(dim(data)[1],dim(data)[2]),DUP=F)
return(euc)
}
...
X<-eucmetric(t(x))
ret<-.Call("get_k_neighbors",X,c(dim(X)[1],dim(X)[2],kpar),DUP=F)
