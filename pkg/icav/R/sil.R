...
euc<-.Call("icav_euc_metric",data, c(dim(data)[1],dim(data)[2]),DUP=F,PACKAGE=icav)
...
sil<-.Call("icav_c_silhouette",DMAT,as.integer(labels),unique(labels),length(labels),DUP=F,PACKAGE=icav)
return(sil)
}
