pknng<-function(X,k,fixed.k=0,diss=F,cte=3,mu="mean",method="euclidean",conn="one",penalize=1,path="dijkstra",MinGroup=0,silent=T){
...
if (method=="euclidean") X<-eucmetric(data)
else if (method=="corr") {
...
if (MinGroup > 0){
addons<-.Call("outlayer_merger",as.list(ret),X,as.integer(c(dim(X)[1], dim(X)[2], conntype, penalize, metric, cte, MinGroup)),DUP=F,PACKAGE="pknng")
